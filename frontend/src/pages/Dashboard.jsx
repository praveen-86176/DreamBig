...
const Navbar = ({ toggleTheme, isDark, onOpenHistory, onLogout }) => (
    <nav className="w-full h-14 border-b border-[var(--border-light)] flex items-center justify-between px-4 md:px-8 bg-[var(--bg-page)]/80 backdrop-blur-md sticky top-0 z-50 transition-colors">
        ...
        <div className="flex items-center gap-2">
            <button onClick={toggleTheme} className="n-btn-ghost p-1.5 hover:bg-[var(--bg-card-hover)] rounded-full transition-all" title="Toggle Theme">
                {isDark ? <Sun className="w-4 h-4" /> : <Moon className="w-4 h-4" />}
            </button>
            <button onClick={onOpenHistory} className="n-btn-ghost p-1.5 hover:bg-[var(--bg-card-hover)] rounded-full transition-all" title="History">
                <HistoryIcon className="w-4 h-4" />
            </button>
            <div className="h-4 w-px bg-[var(--border-light)] mx-1"></div>
            <button onClick={onLogout} className="n-btn-ghost p-1.5 hover:bg-red-500/10 hover:text-red-500 rounded-full transition-all" title="Sign Out">
...
const Dashboard = ({ user, onLogout }) => {
    ...
    return (
        <div className="min-h-screen text-[var(--text-main)] transition-colors duration-500 relative bg-[var(--bg-page)] scroll-smooth overflow-x-hidden">
            <BackgroundBlobs />
            <Navbar toggleTheme={toggleTheme} isDark={isDark} onOpenHistory={() => { fetchHistory(); setHistoryOpen(true); }} onLogout={onLogout} />
